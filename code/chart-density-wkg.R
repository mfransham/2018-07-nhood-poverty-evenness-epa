...
# calculate densities
dens05 <- density(log2(
  filter(poverty, date==2005)$wkgpov.sc / filter(povrates, date==2005)$povrate.wkg), 
  from=-4, to=4)
dens08 <- density(log2(
  filter(poverty, date==2008)$wkgpov.sc / filter(povrates, date==2008)$povrate.wkg), 
  from=-4, to=4)
dens12 <- density(log2(
  filter(poverty, date==2012)$wkgpov.sc / filter(povrates, date==2012)$povrate.wkg), 
  from=-4, to=4)

# gather as data.frame (output as lists)
densxy.wkg <- as.data.frame(cbind(dens05$x, dens05$y, dens08$y, dens12$y))
names(densxy.wkg) <- c("x","y05","y08","y12")
rm(dens05, dens08, dens12)

# calculate difference in densities
densxy.wkg <- mutate(densxy.wkg, 
                 chg08 = y08-y05,
                 chg12 = y12-y08) %>% 
  melt(id.vars = "x", measure.vars = c("y05", "y08", "y12", "chg08", "chg12"), value.name = "value")

# plot
ggplot(filter(densxy.wkg, variable %in% c("y05","y12","chg08","chg12")), 
